...
const BOTTOM_BAR_BG_PATH := "res://assets/generated/afk_rpg_formal/ui/sliced/battle_bar_bg.png"
const BAR_WIDTH := 740.0
...
var bottom_bar_bg: TextureRect
var hp_orb_label: Label
var mp_orb_label: Label


func _ready() -> void:
	target_card.visible = false
	battle_safe_frame.visible = false
	_setup_bottom_action_bar()
	_apply_card_art()
	_apply_slot_icons()
...
func _on_hp_changed(current_hp: float, max_hp: float) -> void:
	...
	if hp_orb_label:
		hp_orb_label.text = "%d%%" % int(current_hp / max_hp * 100.0)
...
func _setup_bottom_action_bar() -> void:
	bottom_bar_bg = TextureRect.new()
	bottom_bar_bg.name = "BottomBarBg"
	bottom_bar_bg.texture = _load_runtime_texture(BOTTOM_BAR_BG_PATH)
	bottom_bar_bg.expand_mode = TextureRect.EXPAND_IGNORE_SIZE
	bottom_bar_bg.stretch_mode = TextureRect.STRETCH_KEEP_ASPECT_CENTERED
	bottom_bar_bg.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(bottom_bar_bg)
	move_child(bottom_bar_bg, 0)

	hp_orb_label = Label.new()
	hp_orb_label.name = "HpOrbLabel"
	hp_orb_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	hp_orb_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	hp_orb_label.add_theme_font_size_override("font_size", 16)
	hp_orb_label.add_theme_color_override("font_color", Color(1.0, 0.92, 0.82))
	hp_orb_label.text = "100%"
	hp_orb_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(hp_orb_label)

	mp_orb_label = Label.new()
	mp_orb_label.name = "MpOrbLabel"
	mp_orb_label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	mp_orb_label.vertical_alignment = VERTICAL_ALIGNMENT_CENTER
	mp_orb_label.add_theme_font_size_override("font_size", 16)
	mp_orb_label.add_theme_color_override("font_color", Color(0.82, 0.92, 1.0))
	mp_orb_label.text = "100%"
	mp_orb_label.mouse_filter = Control.MOUSE_FILTER_IGNORE
	add_child(mp_orb_label)
...
func _apply_hud_layout() -> void:
	...
	if bottom_bar_bg:
		...
		var bar_y := viewport_size.y - BAR_HEIGHT
		bottom_bar_bg.position = Vector2(bar_x, bar_y)
		bottom_bar_bg.size = Vector2(BAR_WIDTH, BAR_HEIGHT)
		if hp_orb_label:
			hp_orb_label.position = Vector2(bar_x + 52.0, bar_y + 78.0)
			hp_orb_label.size = Vector2(80.0, 48.0)
		if mp_orb_label:
			mp_orb_label.position = Vector2(bar_x + BAR_WIDTH - 132.0, bar_y + 78.0)
			mp_orb_label.size = Vector2(80.0, 48.0)

	if main_nav_bar:
		main_nav_bar.offset_top = -(BAR_HEIGHT + 164.0)
		main_nav_bar.offset_bottom = -(BAR_HEIGHT + 8.0)
